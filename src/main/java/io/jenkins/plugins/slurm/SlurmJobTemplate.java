...
public class SlurmJobTemplate extends AbstractDescribableImpl<SlurmJobTemplate> {
    ...
    /**
     * Selects which launch strategy the agent uses: {@code "NATIVE"} (host, no container) or
     * {@code "PYXIS"} (Enroot/Pyxis container).  Null means "infer from existing fields" for
     * backwards-compatibility with templates saved before this field was introduced.
     */
    ...
    /**
     * Returns the explicit launch mode ({@code "NATIVE"} or {@code "PYXIS"}).
     *
     * <p>When no mode has been explicitly saved (legacy templates), the mode is
     * inferred from whichever config object is populated: if a Pyxis configuration
     * with a container image is present it is assumed the user wanted Pyxis;
     * otherwise native launch is assumed.
     */
    @NonNull
    public String getLaunchMode() {
        ...
        // Backwards-compat inference for templates created before this field existed.
        return (pyxis != null && pyxis.isConfigured()) ? "PYXIS" : "NATIVE";
    }

    @DataBoundSetter
    public void setLaunchMode(String launchMode) {
        this.launchMode = "PYXIS".equals(launchMode) ? "PYXIS" : "NATIVE";
    }

    /** Convenience for Jelly EL: {@code ${instance.nativeLaunch}}. */
    public boolean isNativeLaunch() {
        return "NATIVE".equals(getLaunchMode());
    }

    /** Convenience for Jelly EL: {@code ${instance.pyxisLaunch}}. */
    public boolean isPyxisLaunch() {
        return "PYXIS".equals(getLaunchMode());
    }
    ...
    public String toString() {
        return String.format(
                "SlurmJobTemplate[name=%s, label=%s, partition=%s, cpus=%d, memory=%dMB, tres=%s]",
     ...
     */
    @RequirePOST
    public HttpResponse doConfigSubmit(
            org.kohsuke.stapler.StaplerRequest req,
            org.kohsuke.stapler.StaplerResponse rsp,
            @AncestorInPath SlurmCloud owner)
            throws Exception {
        if (owner == null) {
    ...
    public static class DescriptorImpl extends Descriptor<SlurmJobTemplate> {
        ...
        public FormValidation doCheckMinimumNodes(@QueryParameter String value) {
            try {
                ...
                if (nodes > 1) {
                    return FormValidation.warning(
                            "Jenkins agents typically run on single nodes. Multi-node jobs require special configuration.");
                }
                return FormValidation.ok();
            } catch (NumberFormatException e) {
                return FormValidation.error("Invalid number");
        ...
        public FormValidation doCheckTasks(@QueryParameter String value) {
            try {
                ...
                if (tasks > 1) {
                    return FormValidation.warning(
                            "Jenkins agents typically run as single tasks. Multiple tasks require special configuration.");
                }
                return FormValidation.ok();
            } catch (NumberFormatException e) {
                return FormValidation.error("Invalid number");
