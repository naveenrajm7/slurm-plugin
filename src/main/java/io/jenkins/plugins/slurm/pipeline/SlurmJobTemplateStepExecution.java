...
public class SlurmJobTemplateStepExecution extends StepExecution implements Serializable {
    ...
    public boolean start() throws Exception {
        ...
        if (!StringUtils.isEmpty(template.getLabel())) {
            LOGGER.log(Level.INFO, "Adding temporary template to cloud ''{0}'' with label: ''{1}''", new Object[] {
                cloud.name, template.getLabel()
            });
            LOGGER.log(Level.INFO, "Template details: name={0}, partition={1}, cpus={2}, memory={3}", new Object[] {
                template.getName(), template.getPartition(), template.getCpusPerTask(), template.getMemoryPerNode()
            });
            cloud.addDynamicTemplate(template);
            addedTemplate = true;
            LOGGER.log(
                    Level.INFO,
                    "Template added successfully. Total templates in cloud: {0}",
        ...
        try {
            ...
            BodyInvoker bodyInvoker = getContext()
                    .newBodyInvoker()
    ...
    private static class TemplateCleanupCallback extends BodyExecutionCallback {
        ...
        private void cleanup() {
            if (wasAdded && templateId != null && cloudName != null) {
                LOGGER.log(Level.FINE, "Removing temporary template id={0} (label={1}) from cloud: {2}", new Object[] {
                    templateId, templateLabel, cloudName
                });
                ...
                for (SlurmCloud cloud : jenkins.clouds.getAll(SlurmCloud.class)) {
                    if (cloudName.equals(cloud.name)) {
                        ...
                        if (removed) {
                            LOGGER.log(
                                    Level.INFO,
                                    "Removed temporary template id={0} (label={1}) from cloud: {2}",
                                    new Object[] {templateId, templateLabel, cloudName});
                        } else {
                            LOGGER.log(
                                    Level.WARNING,
                                    "Could not find template id={0} (label={1}) in cloud: {2} for cleanup",
                                    new Object[] {templateId, templateLabel, cloudName});
                        }
                        break;
                    }
