...
public class SlurmAgent extends AbstractCloudSlave implements TrackedItem {
    ...
    public SlurmAgent(
        ...
        this.cloudStatsId = cloudStatsId != null ? cloudStatsId : new Id(cloudName, templateId, name);

        LOGGER.log(Level.INFO, "Created Slurm agent: {0} (cloud={1}, template={2}, partition={3})", new Object[] {
            name, cloudName, templateId, partition
        });
    }
    ...
    public void setSlurmJobId(@NonNull String slurmJobId) {
        ...
        LOGGER.log(Level.FINE, "Set Slurm job ID for agent {0}: {1}", new Object[] {getNodeName(), slurmJobId});
    }
    ...
    public void setNodeList(@NonNull String nodeList) {
        ...
        LOGGER.log(Level.FINE, "Set node list for agent {0}: {1}", new Object[] {getNodeName(), nodeList});
    }
    ...
    protected void _terminate(@NonNull TaskListener listener) throws IOException, InterruptedException {
        LOGGER.log(Level.INFO, "Terminating Slurm agent: {0} (job={1})", new Object[] {getNodeName(), slurmJobId});
        ...
        if (computer instanceof hudson.slaves.SlaveComputer) {
            ...
            if (rawLauncher instanceof SlurmLauncher) {
                ((SlurmLauncher) rawLauncher)
                        .setProblem(new java.io.IOException("Agent terminated by retention strategy"));
    ...
    public String toString() {
        return String.format("SlurmAgent[name=%s, job=%s, cloud=%s]", getNodeName(), slurmJobId, cloudName);
    }
