...
- [ ] **Federated `getJobStatus()` reports coarse state (validated on a federated cluster, PR #28 follow-up)** (`SlurmClient.getJobStatus` / `resolveJobState`)
...
- [ ] **Multi-agent-per-node host collisions (native launch)** — see [`docs/NATIVE_LAUNCH_MIGRATION_PLAN.md`](docs/NATIVE_LAUNCH_MIGRATION_PLAN.md) (validated on an 8-GPU node during the native-launch stopgap)
  - Native launch packs N agents on one node; anything bound/locked/written at host-global scope collides (old model was 1 machine = 1 build).
  - **[HIGH]** CK sccache/stunnel fixed localhost ports (`4226` / `127.0.0.1:6379`) collide under CK's `--network=host` → 2nd concurrent CK build dies `Server startup failed: Address in use`. Stopgap: run with `USE_SCCACHE=false`. Proper: unique `SCCACHE_SERVER_PORT` + stunnel `accept` per agent, or drop `--network=host`. MIOpen unaffected (ccache has no daemon). Plugin could expose a per-agent index/unique-port env for workloads to consume.
  - **[MED]** Git ref-repo mirror race (CK + MIOpen): shared `/var/jenkins/ref-repo` guarded by a **per-`NODE_NAME`** lock; N node names on one shared FS don't serialize → concurrent `git remote update` can corrupt. Fix: host-wide lock label or per-agent ref-repo path.
  - **[MED]** Memory not scheduler-enforced when the cluster select plugin is CPU-only (`CR_CPU`): `--mem` is a no-op (`AllocMem=0`), no per-agent RAM cap; runaway build can OOM-kill the agent JVM ("Connection was broken"). CPU (1/N) and GPU (gres:1) isolation do work. Needs cluster `CR_CPU_Memory` + cgroup `ConstrainRAMSpace`.
