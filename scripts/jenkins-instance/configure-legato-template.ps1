# Configure legato-style Slurm template + env-var smoke job.
#
# Usage:
#   pwsh -File scripts/jenkins-instance/configure-legato-template.ps1
...
Write-Host '=== Configure legato migration template + job ==='
...
$groovyPath = Join-Path $PSScriptRoot 'configure-legato-template.groovy'
$groovy = Expand-JobTemplate -TemplatePath $groovyPath -Cfg $cfg
$result = Invoke-JenkinsScript -Jenkins $jenkins -Groovy $groovy
if ("$result" -notmatch 'legato-template-ok') { throw "Template configure failed: $result" }
Write-Host $result
...
$scriptText = Expand-JobTemplate -TemplatePath "$PSScriptRoot\jobs\legato-env-smoke.groovy" -Cfg $cfg
